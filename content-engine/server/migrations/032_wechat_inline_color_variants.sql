WITH updated AS (
  SELECT
    ...
    jsonb_set(
      rules_json,
      '{layout,inlineVariant}',
      to_jsonb(CASE
        WHEN rules_json #>> '{layout,headingVariant}' = 'shadow-card' THEN 'dual'
        WHEN rules_json #>> '{layout,headingVariant}' = 'numbered' THEN 'accent'
        WHEN rules_json #>> '{layout,headingVariant}' = 'band' THEN 'marker'
        WHEN rules_json #>> '{layout,headingVariant}' = 'center-underline' THEN 'mono'
        WHEN rules_json #>> '{layout,headingVariant}' = 'stamp' THEN 'mono'
        ELSE 'accent'
      END),
      true
    ) AS rules_json
...
WITH current_rules AS (
  SELECT
    ...
    jsonb_set(
      ...
      '{layout,inlineVariant}',
      to_jsonb(CASE
        WHEN version.rules_json #>> '{layout,headingVariant}' = 'shadow-card' THEN 'dual'
        WHEN version.rules_json #>> '{layout,headingVariant}' = 'numbered' THEN 'accent'
        WHEN version.rules_json #>> '{layout,headingVariant}' = 'band' THEN 'marker'
        WHEN version.rules_json #>> '{layout,headingVariant}' = 'center-underline' THEN 'mono'
        WHEN version.rules_json #>> '{layout,headingVariant}' = 'stamp' THEN 'mono'
        ELSE 'accent'
      END),
      true
    ) AS rules_json,
