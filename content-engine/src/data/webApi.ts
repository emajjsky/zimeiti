...
export const webCreative = {
  ...
  generateImage: (projectId: string, input: { platform: CreativePlatform; prompt: string; size: '1:1' | '3:4' | '4:3' | '9:16' | '16:9'; negativePrompt?: string }) => request<{ reference: ProjectReference }>(`/creative/projects/${encodeURIComponent(projectId)}/visual/generate`, { method: 'POST', body: JSON.stringify(input) }),
  ...
  saveVisual: (projectId: string, input: { platform: CreativePlatform; planVersion: number; coverReferenceId: string | null; assetReferenceIds: string[]; plan: CreativeVisualPlanItem[] }) => request<{ project: ContentProject }>(`/creative/projects/${encodeURIComponent(projectId)}/visual`, { method: 'PUT', body: JSON.stringify(input) }),
