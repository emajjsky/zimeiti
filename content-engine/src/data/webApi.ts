...
export const webCreative = {
  ...
  generateImage: (projectId: string, input: { platform: DraftPlatform; prompt: string; size: '1:1' | '3:4' | '4:3' | '9:16' | '16:9'; assetIds?: string[] }) => request<{ asset: WorkspaceAsset; projectAsset: ProjectAsset; policy: { scope: 'TEXT_TO_IMAGE' | 'IMAGE_TO_IMAGE'; provider: string; model: string } }>(`/creative/projects/${encodeURIComponent(projectId)}/visual/generate`, { method: 'POST', body: JSON.stringify(input) }),
