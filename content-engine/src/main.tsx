...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {view === 'automation' && <WebSearchPanel preset={searchPreset} onSave={saveSearchCandidate} onNavigate={setView} />}
...
function Discover({ item, intelligence, sources: configuredSources, topics, projects, onSelect, onCreateTopic, onRefresh, refreshFeedback, analysisFeedback, onAnalyze }: { item?: LocalState['intelligence'][number]; intelligence: LocalState['intelligence']; sources: LocalState['sources']; topics: LocalState['topics']; projects: LocalState['projects']; onSelect: (id: string) => void; onCreateTopic: () => void; onRefresh: () => void; refreshFeedback: { status: 'idle' | 'running' | 'success' | 'empty' | 'error'; message: string }; analysisFeedback: { status: 'idle' | 'running' | 'error'; message: string }; onAnalyze: () => void }) {
  ...
  const keywordOptions = [...new Set(intelligence.filter((signal) => isWithinRange(signal.publishedAt)).flatMap((signal) => signal.keywords ?? []))].sort((left, right) => left.localeCompare(right, 'zh-CN'));
  const visible = intelligence.filter((signal) => {
    ...
    return isWithinRange(signal.publishedAt) && (category === 'ALL' || signal.category === category) && (keyword === 'ALL' || signal.keywords?.includes(keyword)) && (source === 'ALL' || signal.source === source) && (language === 'ALL' || signalLanguage === language) && (!query.trim() || text.includes(query.trim().toLocaleLowerCase()));
  });
  ...
  const keywordTags = (signal: LocalState['intelligence'][number]) => {
    if (signal.keywords?.length) return signal.keywords.slice(0, 2);
...
function SettingsHub({ sources, onAddSource, onAddSources, onRemoveSource, onOpenClip, onOpenSearch }: { sources: IntelligenceSource[]; onAddSource: (source: Omit<IntelligenceSource, 'id' | 'lastSyncedAt' | 'lastError'>) => void; onAddSources: (sources: Omit<IntelligenceSource, 'id' | 'lastSyncedAt' | 'lastError'>[]) => void; onRemoveSource: (id: string) => void; onOpenClip: () => void; onOpenSearch: (channel: AssistedChannel) => void }) {
  ...
  const togglePreset = (presetUrl: string) => setSelectedUrls((current) => current.includes(presetUrl) ? current.filter((item) => item !== presetUrl) : [...current, presetUrl]);
  const addSelected = () => {
    const selected = automaticSourceGroups.flatMap((group) => group.sources).filter((source) => selectedUrls.includes(source.url) && !existingUrls.has(source.url));
  ...
  const submit = (event: React.FormEvent) => {
    ...
    onAddSource({ name: name.trim() || '未命名 RSS 源', type: 'RSS', url: url.trim(), category, includeKeywords: toKeywords(includeKeywords), excludeKeywords: toKeywords(excludeKeywords), language, enabled: true, refreshMinutes: 60, trust: '待核验' });
    setName(''); setUrl(''); setIncludeKeywords(''); setExcludeKeywords(''); setError('');
  };
  const formatSyncTime = (value?: string) => value ? new Date(value).toLocaleString('zh-CN', { hour12: false }) : '尚未刷新';

  return <><PageHeader eyebrow="DISCOVER / 情报源" title="资讯来源" /><div className="source-mode-tabs" role="tablist"><button type="button" role="tab" aria-selected={mode === 'automatic'} className={mode === 'automatic' ? 'active' : ''} onClick={() => setMode('automatic')}>自动来源</button><button type="button" role="tab" aria-selected={mode === 'assisted'} className={mode === 'assisted' ? 'active' : ''} onClick={() => setMode('assisted')}>辅助渠道</button></div>{mode === 'automatic' ? <div className="source-workspace"><section className="source-catalog"><header><div><h2>来源目录</h2><span>{automaticSourceGroups.flatMap((group) => group.sources).length} 个可选</span></div><button className="button primary" type="button" disabled={!selectedUrls.length} onClick={addSelected}><Plus size={16}/>添加所选来源</button></header><div className="automatic-source-groups">{automaticSourceGroups.map((group) => <section className="automatic-source-group" key={group.id}><h3>{group.label}<span>{group.sources.length}</span></h3><div>{group.sources.map((preset) => { const connected = existingUrls.has(preset.url); return <label className={connected ? 'connected' : ''} key={preset.url}><input type="checkbox" checked={connected || selectedUrls.includes(preset.url)} disabled={connected} onChange={() => togglePreset(preset.url)} /><span><b>{preset.name}</b><small>{preset.category} · {preset.language === 'ZH' ? '中文' : '英文'}</small></span><em>{connected ? '已接入' : '待添加'}</em></label>; })}</div></section>)}</div></section><details className="custom-rss-panel"><summary>添加自定义 RSS</summary><form className="source-form compact" onSubmit={submit}><label>来源名称<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>RSS 地址<input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://..." /></label><label>归属题材<select value={category} onChange={(event) => setCategory(event.target.value)}>{intelligenceCategories.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>语言<select value={language} onChange={(event) => setLanguage(event.target.value as IntelligenceSource['language'])}><option value="ALL">全部</option><option value="ZH">中文</option><option value="EN">英文</option></select></label><label>包含词<input value={includeKeywords} onChange={(event) => setIncludeKeywords(event.target.value)} /></label><label>排除词<input value={excludeKeywords} onChange={(event) => setExcludeKeywords(event.target.value)} /></label>{error && <p className="form-error">{error}</p>}<button className="button primary" type="submit"><Plus size={16}/>添加来源</button></form></details><section className="connected-sources"><header><h2>已接入</h2><span>{sources.length}</span></header>{sources.length === 0 ? <p className="source-empty">尚未添加来源</p> : <div>{sources.map((source) => <article key={source.id}><span className={`source-state ${source.lastError ? 'error' : source.lastSyncedAt ? 'ready' : ''}`} /><div><b>{source.name}</b><small>{source.category} · {formatSyncTime(source.lastSyncedAt)}</small>{source.lastError && <em>{source.lastError}</em>}</div><button className="icon-button danger" type="button" aria-label={`移除 ${source.name}`} title="移除来源" onClick={() => onRemoveSource(source.id)}><Trash2 size={16}/></button></article>)}</div>}</section></div> : <section className="assisted-channel-grid">{assistedChannels.map((channel) => <article className="assisted-channel-card" key={channel.id}><header><span>{channel.label.slice(0, 1)}</span><div><h2>{channel.label}</h2><small>{channel.domains.join(' · ')}</small></div></header><footer><button className="button" type="button" onClick={onOpenClip}><Compass size={16}/>剪藏链接</button><button className="button primary" type="button" onClick={() => onOpenSearch(channel)}><Search size={16}/>搜索</button></footer></article>)}</section>}</>;
}
