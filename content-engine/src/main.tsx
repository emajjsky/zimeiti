...
type View = 'today' | 'discover' | 'sources' | 'clip' | 'plan' | 'topicEditor' | 'create' | 'publish' | 'review' | 'assets' | 'automation' | 'searchConfig' | 'models' | 'settings';
...
const utilityNav: { view: View; label: string; icon: typeof CalendarDays }[] = [
  ...
  { view: 'searchConfig', label: '检索 API', icon: KeyRound },
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {view === 'discover' && <Discover item={selectedIntel} intelligence={state.intelligence} topics={state.topics} projects={state.projects} onSelect={setSelectedIntelId} onCreateTopic={createTopicFromIntel} onRefresh={refreshRss} refreshFeedback={refreshFeedback} analysisFeedback={analysisFeedback} onAnalyze={analyzeIntelligence} />}
      ...
      {view === 'searchConfig' && <WebSearchSettings />}
...
function Discover({ item, intelligence, topics, projects, onSelect, onCreateTopic, onRefresh, refreshFeedback, analysisFeedback, onAnalyze }: { item?: LocalState['intelligence'][number]; intelligence: LocalState['intelligence']; topics: LocalState['topics']; projects: LocalState['projects']; onSelect: (id: string) => void; onCreateTopic: () => void; onRefresh: () => void; refreshFeedback: { status: 'idle' | 'running' | 'success' | 'empty' | 'error'; message: string }; analysisFeedback: { status: 'idle' | 'running' | 'error'; message: string }; onAnalyze: () => void }) {
...
function ModelSettingsScreen() {
  const [screen, setScreen] = useState<'bailian' | 'agent' | 'connections' | 'policies' | 'usage' | 'editor'>('bailian');
  const [connections, setConnections] = useState<ModelConnection[]>([]);
  const [bailian, setBailian] = useState<BailianCliStatus>(emptyBailianStatus);
...
function WebSearchSettings() {
  ...
  return <><PageHeader eyebrow="DISCOVER / 检索 API" title="网页检索配置" /><section className="web-search-settings"><div className="web-search-settings-head"><div><h2>Tavily</h2><small>{configured ? 'Key 已保存' : '尚未配置 Key'}</small></div><span className={`chip ${configured ? 'mint' : 'yellow'}`}>{configured ? '已配置' : '待配置'}</span></div><div className="web-search-key-form"><label>Tavily API Key<input type="password" value={apiKey} onChange={(event) => { setApiKey(event.target.value); setNotice(''); }} placeholder={configured ? '输入新 Key 以替换' : '粘贴 Tavily API Key'} autoComplete="off" /></label><button className="button primary" disabled={busy || !apiKey.trim()} onClick={() => void save()}>{busy ? '保存中' : '保存'}</button></div>{notice && <p className={notice === '已保存' ? 'web-search-success' : 'form-error'} aria-live="polite">{notice}</p>}</section></>;
}
