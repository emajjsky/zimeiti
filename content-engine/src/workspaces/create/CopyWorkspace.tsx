...
export function CopyWorkspace({ project, brief, skills, activePlatform, onPlatform, onProjectChange, onSaveBrief, onSaveVersion, onOpenModelSettings, onOpenAgentSettings }: {
  ...
  useEffect(() => { setStrategy(brief); setStrategyState('saved'); }, [brief]);
  useEffect(() => {
  ...
  return <section className="copy-workspace">
    ...
    {strategy && <section className="copy-strategy" aria-labelledby="copy-strategy-title"><header><div><h2 id="copy-strategy-title">写作策略</h2><span>{platformName[activeVersion.platform]}</span></div><button className="text-button" type="button" disabled={strategyState === 'saving'} onClick={() => void saveStrategy()}>{strategyState === 'saving' ? '保存中' : strategyState === 'dirty' ? '保存策略' : '已保存'}</button></header><div className="copy-strategy-fields">{sharedDimensions.map(({ id, label }) => <label key={id}><span>{label}</span><select value={strategy.selectedSkills[id]} onChange={(event) => changeStrategy({ selectedSkills: { ...strategy.selectedSkills, [id]: event.target.value } })}>{(skillGroups.get(id) ?? []).map((skill) => <option key={skill.version.id} value={skill.version.id}>{skill.name}</option>)}</select></label>)}{platformDimensions.map(({ id, label }) => <label key={id}><span>{label}</span><select value={platformStrategy?.[id] ?? ''} onChange={(event) => changeStrategy({ platformSkills: { ...strategy.platformSkills, [activeVersion.platform]: { ...(platformStrategy ?? platformSkills(activeVersion.platform, skills)), [id]: event.target.value } } })}>{(skillGroups.get(id) ?? []).map((skill) => <option key={skill.version.id} value={skill.version.id}>{skill.name}</option>)}</select></label>)}<label><span>目标篇幅</span><input value={strategy.lengthTarget} onChange={(event) => changeStrategy({ lengthTarget: event.target.value })}/></label></div></section>}
    ...
    <div className="copy-workspace-layout">
      ...
      <ProjectAgent projectId={project.id} stage="COPY" platform={activeVersion.platform} selection={selection} blockedReason={strategyState === 'saved' ? undefined : '请先保存写作策略'} refreshToken={refreshToken} onClearSelection={() => setSelection(undefined)} onContextChange={setAgentContext} onArtifactOpen={setCandidate} onArtifactAccepted={() => undefined} onOpenSettings={(target) => target === 'agent' ? onOpenAgentSettings() : onOpenModelSettings()}/>
    </div>
