...
export function CopyWorkspace({ project, brief, briefState, skills, activePlatform, onPlatform, onProjectChange, onSaveBrief, onSaveVersion, onOpenModelSettings, onOpenAgentSettings }: {
  ...
  return <section className="copy-workspace">
    ...
    <div className="copy-workspace-layout">
      ...
      <ProjectAgent projectId={project.id} stage="COPY" platform={activeVersion.platform} selection={selection} blockedReason={strategyState === 'saving' ? '正在保存创作设定' : strategyState === 'error' ? '创作设定保存失败，请重试' : undefined} refreshToken={refreshToken} onClearSelection={() => setSelection(undefined)} onContextChange={setAgentContext} onArtifactOpen={setCandidate} onArtifactAccepted={() => undefined} onOpenSettings={(target) => target === 'agent' ? onOpenAgentSettings() : onOpenModelSettings()}/>
    </div>
