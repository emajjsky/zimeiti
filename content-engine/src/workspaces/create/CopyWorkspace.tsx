...
import type { AccountVoiceProfile, CreativeSkillDefinition, CreativeSkillDimension, ProjectAgentContext, ProjectArtifact, VoiceOffset, WritingBriefInput } from '../../domain/creative';
...
export function CopyWorkspace({ project, draft, brief, briefState, skills, accountVoices, onProjectChange, onDraftChange, onReloadDraft, onSaveBrief, onContinue, onOpenModelSettings, onOpenAgentSettings, onOpenVoiceSettings }: {
  ...
  return <section className="copy-workspace">
    ...
    {strategy && <section className="copy-strategy" aria-labelledby="copy-strategy-title"><header><div><h2 id="copy-strategy-title">写作策略</h2><span>公众号母稿</span></div><div className={`copy-strategy-state ${strategyState}`}>{strategyState === 'saving' ? '正在保存创作设定' : strategyState === 'dirty' ? '准备自动保存' : strategyState === 'saved' ? '已自动保存' : <><span>保存失败</span><button className="text-button" type="button" onClick={() => void saveStrategy()}>重试保存</button></>}</div></header><div className="copy-strategy-fields">{sharedDimensions.map(({ id, label }) => <label key={id}><span>{label}</span><select value={strategy.selectedSkills[id]} onChange={(event) => changeStrategy({ selectedSkills: { ...strategy.selectedSkills, [id]: event.target.value } })}>{(skillGroups.get(id) ?? []).map((skill) => <option key={skill.version.id} value={skill.version.id}>{skill.name}</option>)}</select></label>)}<label><span>目标篇幅</span><input value={platformStrategy?.lengthTarget ?? ''} onChange={(event) => changeStrategy({ platformSkills: { ...strategy.platformSkills, WECHAT: { ...(platformStrategy ?? { LAYOUT: '', CHANNEL: '' }), lengthTarget: event.target.value } } })}/></label></div></section>}
