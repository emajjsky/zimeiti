...
export function ReviewWorkspace({ project, activePlatform, onPlatform, onProjectChange }: {
  ...
  const [checked, setChecked] = useState<string[]>(project.delivery?.review?.acknowledgedFactChecks ?? []);
  ...
  const completed = project.stage === 'COMPLETED' || Boolean(delivery?.review);

  useEffect(() => {
    let cancelled = false;
    webCreative.delivery(project.id).then((result) => { if (!cancelled) { setDelivery(result.delivery); setChecked(result.delivery.review?.acknowledgedFactChecks ?? []); } })
  ...
  const finish = async () => {
    ...
    try { const result = await webCreative.completeReview(project.id, checked); setDelivery(result.delivery); onProjectChange(result.project); }
  ...
  return <section className="review-workspace">
    <header className="delivery-workspace-head"><div><h2>{completed ? '发布包已就绪' : '发布前审核'}</h2><p>{completed ? '下载对应渠道的发布稿，随后可到发布中心排期。' : '确认正文、配图、排版和需要人工核对的事实。'}</p></div>{completed && <ShieldCheck size={26}/>}</header>
    {error && <div className="delivery-error" role="alert">{error}</div>}
    <div className="review-check-grid">
      <article className={project.versions.some((item) => String(item.body ?? '').trim().length >= 80) ? 'passed' : 'failed'}><Check size={16}/><div><b>渠道正文</b><span>{project.versions.filter((item) => item.platform !== 'VIDEO_CHANNEL' && String(item.body ?? '').trim().length >= 80).length} 个版本可用</span></div></article>
      <article className={Object.keys(delivery?.layouts ?? {}).length ? 'passed' : 'failed'}><Check size={16}/><div><b>发布排版</b><span>{Object.keys(delivery?.layouts ?? {}).length} 个版本已生成</span></div></article>
      <article className={delivery?.visual ? 'passed' : 'neutral'}><Check size={16}/><div><b>配图素材</b><span>{delivery?.visual?.assets.length ?? 0} 张已选</span></div></article>
    </div>
