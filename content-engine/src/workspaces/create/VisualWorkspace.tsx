...
export function VisualWorkspace({ project, activePlatform, onPlatform, onProjectChange, onOpenMaterials }: {
  ...
  return <section className="visual-workspace">
    <header className="delivery-workspace-head">
      <div><h2>{activePlatform === 'XIAOHONGSHU' ? '制作图文卡片素材' : `选择${platformName[activePlatform]}配图`}</h2><p>{activePlatform === 'XIAOHONGSHU' ? '先选封面和图文卡片素材，再进入发布排版。' : '从项目素材中选用，第一张作为封面。'}</p></div>
    ...
    <nav className="delivery-platform-tabs" aria-label="当前制作渠道">{project.versions.filter((item): item is typeof item & { platform: CreativePlatform } => item.platform !== 'VIDEO_CHANNEL').map((item) => <button type="button" className={item.platform === activePlatform ? 'active' : ''} key={item.platform} onClick={() => onPlatform(item.platform)}>{platformName[item.platform]}</button>)}</nav>
    ...
    <footer className="delivery-workspace-footer">
      ...
      <div><button className="button" type="button" disabled={busy !== null} onClick={() => void save()}>{busy === 'save' ? <LoaderCircle size={16}/> : <Save size={16}/>}保存</button><button className="button primary" type="button" disabled={busy !== null} onClick={() => void complete()}>{busy === 'complete' ? <LoaderCircle size={16}/> : null}确认素材，进入排版</button></div>
    </footer>
  </section>;
}
