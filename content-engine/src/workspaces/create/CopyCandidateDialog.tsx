...
export function CopyCandidateDialog({ artifact, current, busy, onAccept, onReject, onClose }: {
  ...
  return <div className="copy-candidate-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && busy === 'idle') onClose(); }}>
    <section className="copy-candidate-dialog" role="dialog" aria-modal="true" aria-labelledby="copy-candidate-title">
      <header><div><span>{artifact.platform ? platformName[artifact.platform] : '内容项目'}</span><h2 id="copy-candidate-title">{artifact.type === 'OUTLINE' ? '确认大纲' : '查看修改'}</h2></div><button className="icon-button" type="button" aria-label="关闭" disabled={busy !== 'idle'} onClick={onClose}><X size={18}/></button></header>
      ...
      <footer>{canChange ? <><button className="button" type="button" disabled={busy !== 'idle'} onClick={onReject}>{busy === 'rejecting' ? <LoaderCircle size={16}/> : <Trash2 size={16}/>} {artifact.type === 'OUTLINE' ? '放弃大纲' : '放弃修改'}</button><button className="button primary" type="button" disabled={busy !== 'idle' || (artifact.type === 'OUTLINE' && !selectedTitle)} onClick={() => onAccept(artifact.type === 'OUTLINE' ? selectedTitle : undefined)}>{busy === 'accepting' ? <LoaderCircle size={16}/> : <Check size={16}/>} {artifact.type === 'OUTLINE' ? '采用大纲' : '采用修改'}</button></> : <button className="button primary" type="button" onClick={onClose}>关闭</button>}</footer>
    </section>
  </div>;
}
